...
import { Briefcase, MapPin, Users, Save, Check, ChevronRight, ChevronLeft } from "lucide-react";

const STEPS = [
  { id: "nombre", label: "Identificación", icon: Briefcase, motivational: "Ponle nombre a tu oficina" },
  ...
  { id: "detalles", label: "Detalles", icon: Briefcase, motivational: "Características de la oficina" },
];

interface Props { open: boolean; onOpenChange: (open: boolean) => void; onSave: (data: Record<string, unknown>) => Promise<void>; }

const OficinaWizard = ({ open, onOpenChange, onSave }: Props) => {
  const [step, setStep] = useState(0);
  const [saving, setSaving] = useState(false);
  const [validationError, setValidationError] = useState("");
  const [f, setF] = useState({ nombre_interno: "", superficie_m2: null as number | null, num_despachos: null as number | null, num_banos: null as number | null, tiene_ascensor: false, tiene_aire_acondicionado: false, tiene_calefaccion: false, cuota_comunidad: null as number | null });
  ...
  useEffect(() => { if (!open) { setF({ nombre_interno: "", superficie_m2: null, num_despachos: null, num_banos: null, tiene_ascensor: false, tiene_aire_acondicionado: false, tiene_calefaccion: false, cuota_comunidad: null }); setAddr({ tipo_via: "", direccion_completa: "", numero: "", numero_portal: "", planta: "", puerta: "", urbanizacion: "", municipio: "", provincia: "", comunidad_autonoma: "", codigo_postal: "" }); setTitularidad(defaultTitularidadFields); setStep(0); setValidationError(""); } }, [open]);
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave({
        ...
        superficie_m2: f.superficie_m2, num_despachos: f.num_despachos, num_banos: f.num_banos,
        tiene_ascensor: f.tiene_ascensor, tiene_aire_acondicionado: f.tiene_aire_acondicionado,
        tiene_calefaccion: f.tiene_calefaccion, cuota_comunidad: f.cuota_comunidad,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg p-0 gap-0 overflow-hidden rounded-2xl">
        ...
        <div className="px-6 pb-2 min-h-[280px]">
          ...
          {step === 0 && <TextField label="Nombre interno *" value={f.nombre_interno} onChange={v => set("nombre_interno", v)} placeholder="Oficina Gran Vía, Despacho 3B..." />}
          ...
          {step === 3 && (
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                ...
                <NumberField label="Despachos" value={f.num_despachos} onChange={v => set("num_despachos", v)} placeholder="0" />
                <NumberField label="Baños" value={f.num_banos} onChange={v => set("num_banos", v)} placeholder="0" />
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <SwitchField label="Aire acond." checked={f.tiene_aire_acondicionado} onChange={v => set("tiene_aire_acondicionado", v)} />
                <SwitchField label="Calefacción" checked={f.tiene_calefaccion} onChange={v => set("tiene_calefaccion", v)} />
              </div>
            </div>
          )}
        </div>
        <div className="px-6 pb-6 flex gap-2">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <Button onClick={handleSave} disabled={saving} className="rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 gap-1"><Save size={16} /> {saving ? "Guardando..." : "Guardar oficina"}</Button>
          )}
        </div>
      </DialogContent>
    </Dialog>
