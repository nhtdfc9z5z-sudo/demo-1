...
import { Archive, MapPin, Users, Save, Check, ChevronRight, ChevronLeft } from "lucide-react";

const STEPS = [
  { id: "nombre", label: "Identificación", icon: Archive, motivational: "Ponle nombre a tu trastero" },
  ...
  { id: "detalles", label: "Detalles", icon: Archive, motivational: "Características del trastero" },
];

interface Props { open: boolean; onOpenChange: (open: boolean) => void; onSave: (data: Record<string, unknown>) => Promise<void>; }

const TrasteroWizard = ({ open, onOpenChange, onSave }: Props) => {
  const [step, setStep] = useState(0);
  const [saving, setSaving] = useState(false);
  const [validationError, setValidationError] = useState("");
  const [f, setF] = useState({ nombre_interno: "", superficie_m2: null as number | null, planta_sotano: "", tiene_cerradura: true });
  ...
  useEffect(() => { if (!open) { setF({ nombre_interno: "", superficie_m2: null, planta_sotano: "", tiene_cerradura: true }); setAddr({ tipo_via: "", direccion_completa: "", numero: "", numero_portal: "", planta: "", puerta: "", urbanizacion: "", municipio: "", provincia: "", comunidad_autonoma: "", codigo_postal: "" }); setTitularidad(defaultTitularidadFields); setStep(0); setValidationError(""); } }, [open]);
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave({
        ...
        codigo_postal: addr.codigo_postal || null, superficie_m2: f.superficie_m2, planta_sotano: f.planta_sotano || null, tiene_cerradura: f.tiene_cerradura,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg p-0 gap-0 overflow-hidden rounded-2xl">
        ...
        <div className="px-6 pb-2 min-h-[280px]">
          ...
          {step === 0 && <TextField label="Nombre interno *" value={f.nombre_interno} onChange={v => set("nombre_interno", v)} placeholder="Trastero sótano, Trastero nº3..." />}
          ...
          {step === 3 && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <TextField label="Planta/Sótano" value={f.planta_sotano} onChange={v => set("planta_sotano", v)} placeholder="-1, -2..." />
              </div>
              <SwitchField label="Tiene cerradura" checked={f.tiene_cerradura} onChange={v => set("tiene_cerradura", v)} />
            </div>
          )}
        </div>
        <div className="px-6 pb-6 flex gap-2">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <Button onClick={handleSave} disabled={saving} className="rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 gap-1"><Save size={16} /> {saving ? "Guardando..." : "Guardar trastero"}</Button>
          )}
        </div>
      </DialogContent>
    </Dialog>
