...
import { Building, MapPin, Users, Save, Check, ChevronRight, ChevronLeft } from "lucide-react";

const STEPS = [
  { id: "nombre", label: "Identificación", icon: Building, motivational: "Ponle nombre a tu edificio" },
  ...
  { id: "detalles", label: "Detalles", icon: Building, motivational: "Características del edificio" },
];

interface Props { open: boolean; onOpenChange: (open: boolean) => void; onSave: (data: Record<string, unknown>) => Promise<void>; }

const EdificioWizard = ({ open, onOpenChange, onSave }: Props) => {
  const [step, setStep] = useState(0);
  const [saving, setSaving] = useState(false);
  const [validationError, setValidationError] = useState("");
  const [f, setF] = useState({ nombre_interno: "", superficie_m2: null as number | null, num_plantas: null as number | null, num_viviendas: null as number | null, num_locales: null as number | null, num_garajes: null as number | null, ano_construccion: null as number | null, cuota_comunidad: null as number | null });
  ...
  useEffect(() => { if (!open) { setF({ nombre_interno: "", superficie_m2: null, num_plantas: null, num_viviendas: null, num_locales: null, num_garajes: null, ano_construccion: null, cuota_comunidad: null }); setAddr({ tipo_via: "", direccion_completa: "", numero: "", numero_portal: "", planta: "", puerta: "", urbanizacion: "", municipio: "", provincia: "", comunidad_autonoma: "", codigo_postal: "" }); setTitularidad(defaultTitularidadFields); setStep(0); setValidationError(""); } }, [open]);
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave({
        ...
        numero_portal: addr.numero_portal || null, urbanizacion: addr.urbanizacion || null,
        municipio: addr.municipio || null, provincia: addr.provincia || null, comunidad_autonoma: addr.comunidad_autonoma || null,
        codigo_postal: addr.codigo_postal || null, superficie_m2: f.superficie_m2, num_plantas: f.num_plantas,
        num_viviendas: f.num_viviendas, num_locales: f.num_locales, num_garajes: f.num_garajes,
        ano_construccion: f.ano_construccion, cuota_comunidad: f.cuota_comunidad,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg p-0 gap-0 overflow-hidden rounded-2xl">
        ...
        <div className="px-6 pb-2 min-h-[280px]">
          ...
          {step === 0 && <TextField label="Nombre interno *" value={f.nombre_interno} onChange={v => set("nombre_interno", v)} placeholder="Edificio Mayor 5, Bloque A..." />}
          ...
          {step === 3 && (
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                ...
                <NumberField label="Plantas" value={f.num_plantas} onChange={v => set("num_plantas", v)} placeholder="0" />
                <NumberField label="Año constr." value={f.ano_construccion} onChange={v => set("ano_construccion", v)} placeholder="1990" />
              </div>
              <div className="grid grid-cols-3 gap-3">
                <NumberField label="Viviendas" value={f.num_viviendas} onChange={v => set("num_viviendas", v)} placeholder="0" />
                <NumberField label="Locales" value={f.num_locales} onChange={v => set("num_locales", v)} placeholder="0" />
                <NumberField label="Garajes" value={f.num_garajes} onChange={v => set("num_garajes", v)} placeholder="0" />
              </div>
        ...
        <div className="px-6 pb-6 flex gap-2">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <Button onClick={handleSave} disabled={saving} className="rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 gap-1"><Save size={16} /> {saving ? "Guardando..." : "Guardar edificio"}</Button>
          )}
        </div>
      </DialogContent>
    </Dialog>
