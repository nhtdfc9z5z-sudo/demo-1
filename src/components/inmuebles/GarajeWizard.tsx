...
import { Car, MapPin, Users, Save, Check, ChevronRight, ChevronLeft } from "lucide-react";

const STEPS = [
  { id: "nombre", label: "Identificación", icon: Car, motivational: "Ponle nombre a tu garaje" },
  ...
  { id: "detalles", label: "Detalles", icon: Car, motivational: "Características del garaje" },
];

interface Props { open: boolean; onOpenChange: (open: boolean) => void; onSave: (data: Record<string, unknown>) => Promise<void>; }

const GarajeWizard = ({ open, onOpenChange, onSave }: Props) => {
  const [step, setStep] = useState(0);
  const [saving, setSaving] = useState(false);
  const [validationError, setValidationError] = useState("");
  const [f, setF] = useState({ nombre_interno: "", superficie_m2: null as number | null, num_plazas: 1 as number | null, tipo_plaza: "cerrado", tiene_puerta_automatica: false });
  ...
  useEffect(() => { if (!open) { setF({ nombre_interno: "", superficie_m2: null, num_plazas: 1, tipo_plaza: "cerrado", tiene_puerta_automatica: false }); setAddr({ tipo_via: "", direccion_completa: "", numero: "", numero_portal: "", planta: "", puerta: "", urbanizacion: "", municipio: "", provincia: "", comunidad_autonoma: "", codigo_postal: "" }); setTitularidad(defaultTitularidadFields); setStep(0); setValidationError(""); } }, [open]);
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave({
        ...
        codigo_postal: addr.codigo_postal || null, superficie_m2: f.superficie_m2, num_plazas: f.num_plazas,
        tipo_plaza: f.tipo_plaza, tiene_puerta_automatica: f.tiene_puerta_automatica,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg p-0 gap-0 overflow-hidden rounded-2xl">
        ...
        <div className="px-6 pb-2 min-h-[280px]">
          ...
          {step === 0 && <TextField label="Nombre interno *" value={f.nombre_interno} onChange={v => set("nombre_interno", v)} placeholder="Garaje centro, Plaza nº 45..." />}
          ...
          {step === 3 && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <NumberField label="Nº plazas" value={f.num_plazas} onChange={v => set("num_plazas", v)} placeholder="1" />
              </div>
              <SelectField label="Tipo de plaza" value={f.tipo_plaza} onChange={v => set("tipo_plaza", v)} options={[{ value: "abierto", label: "Abierto" }, { value: "cerrado", label: "Cerrado" }, { value: "cubierto", label: "Cubierto" }]} />
              <SwitchField label="Puerta automática" checked={f.tiene_puerta_automatica} onChange={v => set("tiene_puerta_automatica", v)} />
            </div>
          )}
        </div>
        <div className="px-6 pb-6 flex gap-2">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <Button onClick={handleSave} disabled={saving} className="rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 gap-1"><Save size={16} /> {saving ? "Guardando..." : "Guardar garaje"}</Button>
          )}
        </div>
      </DialogContent>
    </Dialog>
